...
const Discover = () => {
  ...
  const activeTopicStyle = "xl:border-2 hover:bg-primary xl:border-[#40b7de] px-3 py-2 rounded xl:rounded-full flex items-center gap-2 justify-center cursor-pointer text-[#40b7de]"
            ...
            {topics.map((item) => (
                <Link href={`/?topic=${item.name}`} key={item.name}>
                    <div className={topic === item.name ? activeTopicStyle : topicStyle}>
                        <span className='lg:flex items-center content-center'>
                            {item.icon}
                        </span>
